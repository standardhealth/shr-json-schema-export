...
function namespaceToSchema(ns, dataElements, grammarVersions, baseSchemaURL) {
  ...
  for (const def of defs) {
    ...
    if (def.value) {
      let { value, required, tbd, primitive } = convertDefinition(def.value, ns, baseSchemaURL);
      if (required) {
      ...
      if (primitive && (!def.isEntry) && (!def.fields.length)) {
        isTerseElement = true;
        ...
        delete schemaDef.type;
      }
    }
    if (def.fields.length) {
      if (isTerseElement) {
        console.error('Internal error: fields present upon a terse element?', def);
      }
      let qualifiedFields = {};
      let fieldNames = {};
    ...
    if (requiredProperties.length) {
      if (isTerseElement) {
        schemaDef.anyOf[1].required = requiredProperties;
      } else {
...
function convertDefinition(valueDef, enclosingNamespace, baseSchemaURL) {
  ...
  if (valueDef.constructor.name === 'ChoiceValue') {
    ...
    return {value: retValue, required: required, tbd: true, primitive};
  ...
  return {value: retValue, required, tbd: false, primitive};
}
